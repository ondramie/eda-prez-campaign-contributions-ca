...
```{r echo=FALSE,  message=FALSE, warning=FALSE, Summary_of_Data_one}
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, Summary_of_Data_two}
# Summarize the unmodified data set
str(pres_df)
```
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, Summary_IFCs}
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, Summary_of_neg_10K}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, Summary_10k_refund}
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, Summary_max_IFCs}
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, adding_variables}
...
# Add Census Data for Counties
#census <- tbl_df(read.csv('PEP_2014_PEPANNRES_with_ann.csv'))
#census <- select(census, GEO.display.label, respop72014)
#census <- rename(census, c("GEO.display.label" = "county", "respop72014" = "county_pop"))
#census <- separate(census, county, c("county", "state"), sep =",")
#pres_df <- tbl_df(merge(pres_df,census, by = 'county'))
...
```{r echo=FALSE,fig.width=11, fig.height=7, message=FALSE, warning=FALSE,histogram_of_IFCs}
# Histogram of IFCs
...
#p1 + ggtitle("Plot 1: Histogram of IFCs with Median, 75% and 25% Quantile")

p2 <- p1 + coord_cartesian(xlim = c(0,3000))+
  scale_x_discrete(breaks= seq(0,3000,1000))
  
#p2 + ggtitle("Plot 2: Zoom-in Histogram of IFCs with Median, 75% and 25% Quantile")
...
```{r echo=FALSE, fig.width=11, fig.height=7, message=FALSE, warning=FALSE, facet}
...
```{r echo=FALSE, fig.width=11, fig.height=7, message=FALSE, warning=FALSE, facet_two}
...
```{r echo=FALSE, fig.width=11, fig.height=7, message=FALSE, warning=FALSE, facet_three}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, normality}
#density
#grid.arrange( plot(density(pres_df$contb_receipt_amt)),
#             qqnorm(pres_df$contb_receipt_amt), ncol = 2, top = "Blah")
...
```{r echo=FALSE, fig.width=11, fig.height=7, message=FALSE, warning=FALSE, log_tran_and_sine_log_tran_of_IFCs}
...
```{r fig.width=11, fig.height=7, echo=FALSE, message=FALSE, warning=FALSE, IFCs_per_Counties}
# Getting modal peaks
...
p10 <- qplot(county, data = pres_df, fill = cand_pty_aff,
      main = " Plot 10: IFC Log Counts Grouped per County and Split per Party")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
        axis.ticks = element_blank())+
  scale_fill_manual(values=c("#0000FF","#FF0000"),
                    name = "Party",
                    labels=c("Democratic Party",
                             "Republican Party"))+
  scale_y_continuous(trans="log10", name="log counts") 

# sum and median per county per party  
...
p12 <- ggplot(aes(x = county, y = IFC_var, fill = cand_pty_aff), 
             data = stats.county) + 
  geom_bar(stat = "identity", position=position_dodge()) +  
  ggtitle("Plot 12: IFC Log Variance per County and Split per Party")+
...
# Double checking
#by(pres_df$contb_receipt_amt, pres_df$county, sum)
#by(stats.county$IFC_sum,stats.county$cand_pty_aff, summary)
#summary(pres_df$county)
...
```{r fig.width=11, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, IFCs_per_Cand}
...
```{r fig.width=11, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, Counts_per_time}
# quick plot of time of contribution
#qplot(contb_receipt_dt, data = pres_df,
#      main = " Plot 13: Counts per Date of contribution")+
#  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) 
```  
[comment]: <> (I have to correct the time data, which is non-sequential here in plot 13.)  

```{r fig.width=11, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, Fixing_Counts_per_time}
...
#qplot(contb_receipt_dt.x, data = pres_df)+
#  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
...
```{r fig.width=11, fig.height=7, echo=FALSE, message=FALSE, warning=FALSE, Atempt_Reconciling_Negative_FCs}
...
```{r echo=FALSE, fig.width=11, fig.height=7,  message=FALSE, warning=FALSE, univariates}
...
```{r echo=FALSE, fig.width=11, fig.height=7, message=FALSE, warning=FALSE, Cleaning_up_Data}
# (2) and (3)
# Clean up data 
...
if (T) {
  ...
  pres_df.clean$job <- gsub(".*RETIRED.*", "retired", pres_df.clean$job, ignore.case = TRUE)
  ...
  #select(pres_df.clean,contbr_occupation, job)
  pres_df.clean$job <- as.factor((pres_df.clean$job))
  #summary(na.omit(pres_df.clean$job))
  #summary(pres_df.clean$contbr_occupation)
  ...
  #select(net.contbr,contbr_occupation, job)
  net.contbr$job <- as.factor((net.contbr$job))
  #summary(na.omit(net.contbr$job))
  #summary(net.contbr$contbr_occupation)
}

if (FALSE) {
no_info = c("INFORMATION REQUESTED PER BEST EFFORTS", "INFORMATION REQUESTED", "", "NOT AVAILABLE")
unemployed = c("NOT EMPLOYED", "NONE")

pres_df.clean[is.na(pres_df.clean)] <- "N/A"
pres_df.clean$job <- "N/A"
pres_df.clean$contbr_occupation <- as.character(pres_df.clean$contbr_occupation)

for (i in seq_along(pres_df.clean$contbr_occupation)){
  for (j in seq_along(no_info)){
    if (pres_df.clean$contbr_occupation[[i]] == no_info[[j]]) {
      pres_df.clean$job[[i]] <- "N/A" 
    } else pres_df.clean$job[[i]] <- pres_df.clean$contbr_occupation[[i]]
  }
  for (k in seq_along(unemployed)){
    if (pres_df.clean$contbr_occupation[[i]] == unemployed[[k]]) {
      pres_df.clean$job[[i]] <-  "UNEMPLOYED"
    } else pres_df.clean$job[[i]] <- pres_df.clean$contbr_occupation[[i]]
  }
}

desired = c("RETIRED","UNEMPLOYED", "N/A")

for (i in seq_along(pres_df.clean$job)) {
  for (j in seq_along(desired)) {
    if (pres_df.clean$job[[i]] != desired[[j]]) {
       pres_df.clean$job[[i]] <- "EMPLOYED"
    } else pres_df.clean$job[[i]] <- pres_df.clean$job[[i]]
  }
}

head(pres_df.clean$job)

for (i in seq_along(pres_df.clean$contbr_occupation)){
  print(pres_df.clean$contbr_occupation[[i]])
}
}
if (FALSE) {
contbr.occupation.count <- pres_df.clean %>%
...
not_here <- "INFORMATION REQUESTED PER BEST EFFORTS|INFORMATION REQUESTED|NOT AVAILABLE|^$|"
not_working <- "NOT EMPLOYED|NONE"
contbr.occupation.count$jobby <- gsub(not_here, "NA", contbr.occupation.count$contbr_occupation, ignore.case = T)
contbr.occupation.count$jobby <- gsub(not_working, "UNEMPLOYED", contbr.occupation.count$jobby, ignore.case = T)
contbr.occupation.count$jobby_two <- gsub("[^RETIRED|NA|UNEMPLOYED]", "WORKING", contbr.occupation.count$jobby, ignore.case = T)
contbr.occupation.count$jobby <- substr(contbr.occupation.count$jobby , regexpr('WORKING',contbr.occupation.count$jobby), regexpr('WORKING',contbr.occupation.count$jobby)+6)

head(contbr.occupation.count)

summary(contbr.occupation.count$jobby)

}
if (FALSE) {
  for (k in 0:length(pres_df.clean$jobby)){
    if ((k %in% no_info))
    #pres_df.clean$jobby[k]
    #}}
    #for (j in no_info) 
    #pres_df.clean$jobby <- trimws(gsub(j, "N/A", pres_df.clean$contbr_occupation, ignore.case = T))
    #}
    for (k in pres_df.clean$contbr_occupation) {
      if (k != "N/A" | k != "UNEMPLOYED"){
        pres_df.clean$jobby[k] = "EMPLOYED"  
      }
  }
  }            
}

#filter(contbr.occupation.count,contbr_occupation == "N/A")
retired <- pres_df[pres_df$contbr_occupation=="RETIRED",]
#by(retired$contb_receipt_amt, retired$cand_pty_aff,summary)
#by(retired$contb_receipt_amt, retired$cand_pty_aff,sum)
#by(pres_df$contb_receipt_amt, pres_df$cand_pty_aff,sum)

retired_two <- net.contbr[net.contbr$occupation=="RETIRED" & net.contbr$contbr_amt_sum > 0,]
#by(retired_two$contbr_amt_sum, retired_two$party,summary)
#by(retired_two$contbr_amt_sum, retired_two$party,sum)
#contbr.occupation.count[contbr.occupation.count$contbr_occupation == "RETIRED",]
...
#summary statistics of pres_df.clean
#summary(pres_df.clean)
```

### Box Plot - Gender, Party, Gender Information

```{r echo=FALSE,  fig.width=11, fig.height=7, message=FALSE, warning=FALSE, IFC_statistics_per_Gender_or_Party}
...
```{r echo=FALSE, fig.width=11, fig.height=7, message=FALSE, warning=FALSE, IFCs_per_county}
...
```{r echo=FALSE, fig.width=11, fig.height=7, message=FALSE, warning=FALSE, BoxPlot_Job}
...
#### IFCs vs. Time
```{r fig.width=11, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, Categorical_Bivariate}
...
```{r fig.width=11, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, IFC_vs_sum_overplotting_and_jitter}
...
```{r fig.width=11, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, spine}
...
#lablist.x<-as.vector(sort(unique(pres_df.clean$cand_nm)))
#axis(1, at=seq(1, 25, by=1), labels = FALSE)
#text(x = seq(1, 25, by=1), par("usr")[3] - 0.2, 
#labels = lablist.x, srt = 90, pos = 1, xpd =T)
```
...
```{r fig.width=11, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, Multivariate}
getPalette = colorRampPalette(brewer.pal(8, "Dark2"))

x = 0.1

ggplot(data = pres_df.clean, aes(contb_receipt_dt, 
                                 contb_receipt_amt, color = county)) + 
   geom_point(alpha = x, position = position_jitter(h=0)) + 
  scale_y_log10() + scale_color_manual(values = getPalette(58)) +
theme(legend.position = "top") + guides(color=guide_legend(nrow=6)) +
  xlab("Time") + ylab("IFC") + 
  scale_x_date(limits = c(as.Date("2015-06-01"), 
                          as.Date("2015-10-01"))) + 
  ggtitle("Plot 38: IFCs vs. Time - County")

ggplot(data = pres_df.clean, aes(contb_receipt_dt, contb_receipt_amt, 
                                 color = cand_pty_aff)) + 
   geom_point(alpha = x, position = position_jitter(h=0)) + 
  scale_y_log10() + scale_color_manual(values = getPalette(2)) + 
  theme(legend.position = "top") + 
  guides(color=guide_legend(nrow=1)) +
  xlab("Time") + ylab("IFC") + 
  scale_x_date(limits = c(as.Date("2015-06-01"), 
                          as.Date("2015-10-01"))) + 
  ggtitle("Plot 39: IFCs vs. Time - Party")
...
theme(legend.position = "top") + guides(color=guide_legend(nrow=1)) +
  ...
  scale_x_date(limits = c(as.Date("2015-06-01"), 
                          as.Date("2015-10-01"))) + 
  ggtitle("Plot 40: IFCs vs. Time - Gender")
...
theme(legend.position = "top") + guides(color=guide_legend(nrow=3)) +
  xlab("Time") + ylab("IFC") + 
  scale_x_date(limits = c(as.Date("2015-06-01"), 
                          as.Date("2015-10-01"))) + 
  ggtitle("Plot 41: IFCs vs. Time - Candidate")
...
theme(legend.position = "top") + guides(color=guide_legend(nrow=1)) + 
    theme(axis.text.x = element_text(angle = 90, 
                                   ...
                                   vjust = 0.5), 
        axis.ticks = element_blank())+
  ggtitle("Plot 42: IFCs vs. Candidate - Occupation")

# geom_point(alpha = 1/20, position = position_jitter(h=0))
#qplot(x = contb_receipt_amt, data = pres_df.clean, fill = cand_pty_aff)
```
These don't seem to produce anything discernible. 

```{r fig.width=11, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, Density}

#density plots
qplot(contb_receipt_amt, data = na.omit(pres_df.clean), binwidth = 0.01,
...
```
These support the contribution buckets or striations witnessed in other plots, and the finding found in the spline plots. 

```{r fig.width=11, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, jit_and_box}
#jitter plots
qplot(x = county, y = contb_receipt_amt, 
      data = pres_df.clean, color = job) +
  facet_wrap(~cand_pty_aff) + theme(axis.text.x = element_text(angle = 90, 
                                   hjust = 1, 
                                   vjust = 0.5)) + 
  theme(legend.position = "top") + guides(color=guide_legend(nrow=1))
...
qplot(x = county, y = contb_receipt_amt, 
      ...
      fill = cand_pty_aff) +  facet_wrap(~job, nrow = 2) + 
  theme(axis.text.x = element_text(angle = 90, 
                                   hjust = 1, 
                                   vjust = 0.5)) + 
  theme(legend.position = "top") + guides(color=guide_legend(nrow=1))
...
ggplot(na.omit(pres_df.clean), 
  ...
  geom_boxplot() + facet_wrap(~job, nrow = 2) +
    theme(axis.text.x = element_text(angle = 90, 
                                   hjust = 1, 
                                   vjust = 0.5)) +
...
```{r fig.width=11, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, one_ANOVA}
...
```{r fig.width=11, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, chi}
rev_foi <- rev(foi)  
...
### Plot One
```{r fig.width=11, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, Plot_One}
...
### Plot Two
```{r fig.width=12, fig.height=7, echo=FALSE,  message=FALSE, warning=FALSE, Plot_Two}
...
### Plot Three
```{r fig.width=12, fig.height=12, echo=FALSE,  message=FALSE, warning=FALSE, Plot_Three}
